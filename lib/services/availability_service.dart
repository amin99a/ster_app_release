...
class AvailabilityService extends ChangeNotifier {
  ...
  bool _bookingConflictsWith(Booking booking, DateTime startDate, DateTime endDate) {
    return (booking.startDate.isBefore(endDate) && booking.endDate.isAfter(startDate));
  }
  ...
  Map<String, bool> getCarsAvailability(List<String> carIds, DateTime startDate, DateTime endDate) {
  ...
  CarAvailabilityInfo getCarAvailabilityInfo(String carId, DateTime startDate, DateTime endDate) {
    final conflictingBookings = _bookings.where((booking) {
      return booking.carId == carId && 
             booking.status != 'cancelled' &&
             _bookingConflictsWith(booking, startDate, endDate);
  ...
  DateTime? _getNextAvailableDate(String carId, DateTime fromDate) {
    ...
    for (int i = 0; i < maxDaysToCheck; i++) {
      if (isCarAvailable(carId, checkDate, checkDate.add(const Duration(days: 1)))) {
        return checkDate;
      }
  ...
  List<Booking> getActiveBookings(String carId) {
    return _bookings.where((booking) {
  ...
  void _updateAvailability() {
    ...
    for (final carId in allCarIds) {
      ...
      availability[carId] = true; // This would be calculated based on current state
    }
  ...
  Map<DateTime, bool> getAvailabilityCalendar(String carId, {int days = 30}) {
    ...
    for (int i = 0; i < days; i++) {
      ...
      calendar[date] = isCarAvailable(carId, date, date.add(const Duration(days: 1)));
    }
...
class CarAvailabilityInfo {
  ...
  String get nextAvailableMessage {
    if (nextAvailableDate != null) {
      final daysUntilAvailable = nextAvailableDate!.difference(DateTime.now()).inDays;
