...
class SupabaseService {
  ...
  static Future<app_user.User?> getCurrentUser() async {
    try {
      ...
      if (cached != null && cached['timestamp'] > DateTime.now().subtract(_cacheExpiry).millisecondsSinceEpoch) {
  ...
  /// Upload user avatar
  static Future<String?> uploadUserAvatar(String userId, Uint8List imageBytes) async {
    try {
  ...
  static Future<List<Car>> getCars({
    ...
    try {
      ...
      return response.map((car) => Car.fromJson(car)).toList();
    } catch (e) {
  ...
  static Future<String?> createCar(Map<String, dynamic> carData) async {
    try {
      final user = _supabase.auth.currentUser;
      if (user == null) {
      ...
      debugPrint('Car created successfully');
      return response['id'];
    } catch (e) {
  ...
  /// Update car listing
  static Future<bool> updateCar(String carId, Map<String, dynamic> updates) async {
    try {
      updates['updated_at'] = DateTime.now().toIso8601String();
  ...
  static Future<List<Review>> getReviews(String targetId, String targetType) async {
    try {
      final response = await _supabase
          .from('reviews')
          .select('''
  ...
  /// Upload car image
  static Future<String?> uploadCarImage(String carId, Uint8List imageBytes) async {
    try {
      final fileName = 'car_${carId}_${DateTime.now().millisecondsSinceEpoch}.jpg';
  ...
  static void subscribeToNotifications(void Function(Map<String, dynamic>) callback) {
  ...
  static Future<List<app_notification.Notification>> getUserNotifications() async {
    try {
      final user = _supabase.auth.currentUser;
      if (user == null) {
        return [];
      ...
      return response.map((notification) => app_notification.Notification.fromJson(notification)).toList();
    } catch (e) {
  ...
  /// Get chat messages
  static Future<List<Message>> getChatMessages(String otherUserId) async {
    try {
      final user = _supabase.auth.currentUser;
      if (user == null) {
        return [];
  ...
  /// Get user favorites
  static Future<List<Car>> getUserFavorites() async {
    try {
      final user = _supabase.auth.currentUser;
      if (user == null) {
        return [];
      ...
      return response.map((favorite) => Car.fromJson(favorite['cars'])).toList();
    } catch (e) {
  ...
  static void subscribeToChannel(String channelName, void Function(Map<String, dynamic>) callback) {
