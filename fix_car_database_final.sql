-- Complete Fix for Car Database Issues - Final Version
...
DO $$
BEGIN
  -- Drop fuel_type constraint if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'cars_fuel_type_check' 
    AND table_name = 'cars'
  ) THEN
    ALTER TABLE cars DROP CONSTRAINT cars_fuel_type_check;
  END IF;
  
  -- Drop transmission constraint if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'cars_transmission_check' 
    AND table_name = 'cars'
  ) THEN
    ALTER TABLE cars DROP CONSTRAINT cars_transmission_check;
  END IF;
  
  -- Drop category constraint if it exists
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'cars_category_check' 
    AND table_name = 'cars'
  ) THEN
    ALTER TABLE cars DROP CONSTRAINT cars_category_check;
  END IF;
END $$;
